...
export default function SearchPage({ className = "" }) {
  ...
  return (
    <div className={`relative flex flex-1 h-full ${className}`}>
      {/* Sidebar */}
      {/* <aside
        className={`h-full bg-white transition-all duration-200 ease-in-out overflow-hidden
          md:relative md:translate-x-0
          ${display ? "md:w-[23%]" : "md:w-0"}
          absolute md:static top-0 left-0 z-20
          ${display ? "translate-x-0 w-[70%]" : "-translate-x-full w-[70%]"} // overlay on mobile
        `}
      >
      ...
      <aside
        className={`
          ...
          absolute sm:static top-0 left-0 z-20
      ...
      {/* Toggle Button */}
      {/* <button
        className={`absolute top-3 ${display? 'left-[260px] lg:left-[24%]':'left-4'} z-30 px-2 py-2 bg-white rounded shadow-md
                   focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer`}
        onClick={toggleSidebar}
      >
        <FilterIcon className="text-orange-500 hover:text-orange-600" />
